...
exports.getEvents = (0, asyncHandler_1.asyncHandler)((req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const [totalEvents, events] = yield Promise.all([
        db_1.prismaClient.event.count(),
    ...
    // Properly typed image conversion function
    const convertImageToBase64 = (image) => {
        if (!image)
            return null;
        // Handle both Uint8Array from Prisma and Buffer
        const buffer = Buffer.isBuffer(image) ? image : Buffer.from(image.buffer);
        return `data:image/jpeg;base64,${buffer.toString("base64")}`;
    };
    const eventsWithImages = events.map((event) => (Object.assign(Object.assign({}, event), { image: convertImageToBase64(event.image), registrations: event.registrations.map((reg) => ({
            user: Object.assign({}, reg.user),
        })) })));
    res.json({
        success: true,
        totalEvents,
